...
- 修复代码与项目文件内容被汉化（#2）：词典里的单词级 exact 词条（cloud/text/a 等）此前会与编辑器按 token 切分的代码文本节点、文件树里的文件名撞车（如代码中的 `text`→`文本`、`<a />`→`<一个 />`、Java 包 `cloud`→`云端`）。渲染层注入脚本新增「工作区内容禁区」——编辑器代码（.view-lines，含 diff/聊天代码块/Notebook）、文件树、搜索结果、终端输出、面包屑内的文本一律不翻译；界面（设置/菜单/对话框）汉化不受影响
